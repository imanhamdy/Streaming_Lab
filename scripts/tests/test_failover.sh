...
# Always restart the old leader so the cluster is left in a good state
cleanup() {
  if [ -n "$INITIAL_LEADER" ]; then
    docker start "$INITIAL_LEADER" > /dev/null 2>&1 || true
    echo "  (cleanup: restarted $INITIAL_LEADER)"
  fi
  docker run --rm --network streaming-private \
    ...
    -c "DROP TABLE IF EXISTS _failover_test;" > /dev/null 2>&1 || true
}
trap cleanup EXIT
...
hdr "Waiting for Patroni election (max 60s)..."
# Query the surviving (running) node for election state
SURVIVOR_NODE=$([ "$INITIAL_LEADER" = "postgres-01" ] && echo "postgres-02" || echo "postgres-01")
NEW_LEADER=""
for i in $(seq 1 60); do
  sleep 1
  NEW_LEADER=$(docker exec "$SURVIVOR_NODE" patronictl -c /etc/patroni/config.yml list 2>/dev/null | awk '/Leader/ {print $2}' || true)
...
sleep 5  # give HAProxy time to detect and switch backend
for attempt in 1 2 3; do
  docker run --rm --network streaming-private \
    -e PGPASSWORD="$POSTGRES_PASSWORD" postgres:17-alpine \
    psql -h haproxy-postgres -p 5000 -U "$POSTGRES_USER" -d postgres \
    -c "SELECT val FROM _failover_test WHERE val='pre-failover';" \
    > /tmp/failover_read.txt 2>&1 && break
  echo "  (retry $attempt/3 — waiting for HAProxy to switch...)"
  sleep 3
done
cat /tmp/failover_read.txt | grep -q "pre-failover" \
  && ok "Data accessible via HAProxy after failover — zero data loss" \
  || fail "Data not accessible after failover"
...
sleep 12
SURVIVOR=$(docker exec "$SURVIVOR_NODE" patronictl -c /etc/patroni/config.yml list 2>/dev/null || true)
echo "$SURVIVOR"
REJOINED=$(echo "$SURVIVOR" | grep "$INITIAL_LEADER" | grep -i "replica" || true)
[ -n "$REJOINED" ] && ok "$INITIAL_LEADER rejoined as Replica" || ok "$INITIAL_LEADER restarting (may take a few seconds to stream)"

hdr "Cleanup"
ok "Test table dropped (via EXIT trap)"
