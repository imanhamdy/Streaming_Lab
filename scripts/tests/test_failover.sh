...
docker exec postgres-01 patronictl -c /etc/patroni/patroni.yml list 2>/dev/null
INITIAL_LEADER=$(docker exec postgres-01 patronictl -c /etc/patroni/patroni.yml list 2>/dev/null | awk '/Leader/ {print $2}')
...
for i in $(seq 1 30); do
  sleep 1
  NEW_LEADER=$(docker exec postgres-02 patronictl -c /etc/patroni/patroni.yml list 2>/dev/null | awk '/Leader/ {print $2}' || true)
...
sleep 10
docker exec postgres-02 patronictl -c /etc/patroni/patroni.yml list 2>/dev/null
REJOINED=$(docker exec postgres-02 patronictl -c /etc/patroni/patroni.yml list 2>/dev/null | grep "$INITIAL_LEADER" | grep -i "replica" || true)
